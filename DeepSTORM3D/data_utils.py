...
class ImagesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.setup_params['fd_flag']:
            # read coordinate map
            center_idx = self.labels[ID][1]
            coord_np = self.setup_params['xy_maps'][center_idx]
            # turn image into torch tensor with 1 channel
            coord_tensor = torch.from_numpy(coord_np)
            im_coord_tensor = torch.concat((im_tensor, coord_tensor), dim=0)
            im_coord_tensor = im_coord_tensor.type(torch.FloatTensor)
        else:
            im_coord_tensor = im_tensor.type(torch.FloatTensor)

        # corresponding xyz labels turned to a boolean tensor
        xyz_np = self.labels[ID][0]  # xyz
        if xyz_np is not None:
            bool_grid = batch_xyz_to_boolean_grid(xyz_np, self.setup_params)
        else:
            bool_grid = torch.zeros(self.setup_params['D'], self.setup_params['H']*4, self.setup_params['W']*4)    # add sth

        return im_coord_tensor, bool_grid
...
class ExpDataset(Dataset):
    ...
    def __init__(self, im_list, setup_params):
        ...
        self.crop_rcs = setup_params['crop_rcs']
        self.xy_maps = setup_params['xy_maps']
        self.H = setup_params['H']
        self.num_sub_areas = len(setup_params['crop_rcs'])
    ...
    def __getitem__(self, index):
        ...
        # load tif image
        image0 = imread(self.im_list[index]).astype("float32")

        cropped_imxys = torch.zeros(self.num_sub_areas, self.H, self.H)
        for i in range(len(self.crop_rcs)):
            (r, c) = self.crop_rcs[i]
            sub_image = image0[r:r+self.H, c:c+self.H]
            # normalize image according to the training setting
            if self.project_01 is True:
                sub_image = normalize_01(sub_image)
            else:
                sub_image = (sub_image - self.global_factors[0]) / self.global_factors[1]
            # alter the mean and std to match the training set
            if self.project_01 is True:
                sub_image = (sub_image - sub_image.mean()) / sub_image.std()
                sub_image = sub_image * self.train_stats[1] + self.train_stats[0]

            cropped_imxys[i, :, :] = torch.from_numpy(sub_image)

        return cropped_imxys, image0

class ExpDataset_(Dataset):
    ...
    def __init__(self, im_list, setup_params):
        ...
        self.crop_rc_start = setup_params['crop_rc_start']
        self.xy_maps = setup_params['xy_maps']
        self.H = setup_params['H']
    ...
    def __getitem__(self, index):
        ...
        cropped_imxys = np.zeros(len(self.crop_rc_start), 3, self.H, self.H)
        for i in range(len(self.crop_rc_start)):
            (r, c) = self.crop_rc_start[i]
            sub_im = im_np[r:r+self.H, c:c+self.H]
            im_np = sub_im
            # normalize image according to the training setting
            if self.project_01 is True:
            ...
            # turn image into torch tensor with 1 channel
            im_np = np.expand_dims(im_np, 0)
            xy_map = self.xy_maps[i]
            cropped_imxys[i, :, :, :] = np.concatenate((im_np, xy_map), axis=0)

            im_tensor = torch.from_numpy(cropped_imxys)
