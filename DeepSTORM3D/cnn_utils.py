...
class LocalizationCNN_(nn.Module):
    def __init__(self, setup_params):
        super(LocalizationCNN_, self).__init__()
        self.norm = nn.BatchNorm2d(num_features=1, affine=True)
        self.layer1 = Conv2DLeakyReLUBN(3, 64, 3, 1, 1, 0.2)
        self.layer2 = Conv2DLeakyReLUBN(64 + 1, 64, 3, 1, 1, 0.2)
        if setup_params['dilation_flag']:
            self.layer3 = Conv2DLeakyReLUBN(64 + 1, 64, 3, (2, 2), (2, 2), 0.2)
            self.layer4 = Conv2DLeakyReLUBN(64 + 1, 64, 3, (4, 4), (4, 4), 0.2)
    ...
    def forward(self, im_coord):
        # extract multi-scale features
        im, coord_map = im_coord[:, 0:1, :, :], im_coord[:, 1:, :, :]
        im = self.norm(im)
        coordcov = torch.concat((im, coord_map), dim=1)
        out = self.layer1(coordcov)
        features = torch.cat((out, im), 1)
        out = self.layer2(features) + out
        features = torch.cat((out, im), 1)
        out = self.layer3(features) + out
...
class LocalizationCNN_2(nn.Module):
    def __init__(self, setup_params):
        super(LocalizationCNN_2, self).__init__()
        self.norm = nn.BatchNorm2d(num_features=3, affine=True)
        self.layer1 = Conv2DLeakyReLUBN(3, 64, 3, 1, 1, 0.2)
        self.layer2 = Conv2DLeakyReLUBN(64 + 3, 64, 3, 1, 1, 0.2)
        if setup_params['dilation_flag']:
            self.layer3 = Conv2DLeakyReLUBN(64 + 3, 64, 3, (2, 2), (2, 2), 0.2)
            self.layer4 = Conv2DLeakyReLUBN(64 + 3, 64, 3, (4, 4), (4, 4), 0.2)
            self.layer5 = Conv2DLeakyReLUBN(64 + 3, 64, 3, (8, 8), (8, 8), 0.2)
            self.layer6 = Conv2DLeakyReLUBN(64 + 3, 64, 3, (16, 16), (16, 16), 0.2)
        else:
            self.layer3 = Conv2DLeakyReLUBN(64 + 3, 64, 3, (2, 2), (2, 2), 0.2)
            self.layer4 = Conv2DLeakyReLUBN(64 + 3, 64, 3, (4, 4), (4, 4), 0.2)
            self.layer5 = Conv2DLeakyReLUBN(64 + 3, 64, 3, 1, 1, 0.2)
            self.layer6 = Conv2DLeakyReLUBN(64 + 3, 64, 3, 1, 1, 0.2)
        self.deconv1 = Conv2DLeakyReLUBN(64 + 3, 64, 3, 1, 1, 0.2)
        self.deconv2 = Conv2DLeakyReLUBN(64, 64, 3, 1, 1, 0.2)
        self.layer7 = Conv2DLeakyReLUBN(64, setup_params['D'], 3, 1, 1, 0.2)
        self.layer8 = Conv2DLeakyReLUBN(setup_params['D'], setup_params['D'], 3, 1, 1, 0.2)
        self.layer9 = Conv2DLeakyReLUBN(setup_params['D'], setup_params['D'], 3, 1, 1, 0.2)
